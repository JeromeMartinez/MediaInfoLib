...
template<size_t N>
struct info_list_block
{
    info_list_size sizes;
    info_list items[N];
}; 

#define INFO_LIST_BLOCK(name, gap, ...) \
constexpr const char* name##_arr[] = { __VA_ARGS__ }; \
constexpr info_list_block<sizeof(name##_arr)/sizeof(name##_arr[0])> name##_storage = { \
    { sizeof(name##_arr)/sizeof(name##_arr[0]), gap }, \
    { __VA_ARGS__ } \
}; \
static info_list const* const name = name##_storage.items;

INFO_LIST_BLOCK(Av1_metadata_type_xxxx, 0,
...
//---------------------------------------------------------------------------
static info_list const Av1_metadata_type_List[] =
{
    "METADATA_TYPE_HDR_CLL",
    "METADATA_TYPE_HDR_MDCV",
    "METADATA_TYPE_SCALABILITY",
...
static info_list_size const Av1_metadata_type_Sizes[] =
{
    { 0, 1 }, // No 0
    { size(Av1_metadata_type_List), 0},
};
/*
struct complex_list {
    const char* const* const List;
};
complex_list Av1_metadata_type{
    Av1_metadata_type_List
};
*/
...
void File_Av1::metadata()
{
    ...
    auto C = Av1_metadata_type_xxxx[-1];
    auto A = Av1_metadata_type_xxxx[0];
    auto B = Av1_metadata_type_xxxx[1];

    Get_leb128 (metadata_type,                                  "metadata_type"); ParamElement_Info3C(metadata_type <= numeric_limits<int32u>::max(), static_cast<int32u>(metadata_type), Av1_metadata_type_List, Av1_metadata_type_Sizes);
